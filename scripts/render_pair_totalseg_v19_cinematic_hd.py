"""
render_pair_totalseg_v19_cinematic_hd.py — Physically Grounded Baseline

v19 removes all procedural noise and heuristic modulation from the material.
The goal: every parameter is either measured, calibrated, or geometry-driven.
No stochastic fields. No assumption-driven proxies.

Removed entirely from v18:
  - Multi-scale roughness noise (mid/macro/micro stacked noise fields → no anatomical basis)
  - AO curvature roughness modulation (AO ≠ curvature; AO ≠ surface wetness)
  - AO coat weight boost (AO ≠ fluid retention)
  - Coat noise modulation (stochastic wetness variation, not physically motivated)
  - Gravity moisture gradient (object-space Z ≠ fluid accumulation topology)
  - Coat normal perturbation (even at 0.02 — coherent highlight flow is correct for wet tissue)
  - Macro deformation bump noise (scanner geometry is ground truth; noise corrupts it)
  - Unused map_node (was feeding roughness noise only)

What remains is physically grounded:
  - IOR per tissue (from TISSUES table, calibrated)
  - Roughness per tissue (static, calibrated value — no modulation)
  - SSS: Random Walk, static scale, per-tissue calibrated radius and weight
  - Coat: static weight and roughness per tissue — broad coherent specular film
  - AO contact shadow on base color only (valid local occlusion approximation)
  - Low-freq vascular color variation: ONLY remaining noise — models lobular blood content
    variation in liver/kidney/spleen/heart. Affects base color only (not normals/roughness).
    Anatomically motivated: hepatic lobules create visible patchwork in perfused tissue.
  - Bevel: valid edge softening for STL-derived meshes
  - Texture bump: only if physical bump file exists (geometry-derived structure)

Run:
    blender --background --python scripts/render_pair_totalseg_v19_cinematic_hd.py -- \\
...
def main():
    ...
    print(f"\n{'='*65}")
    print(f"[v19-cinematic-hd]  subject={args.subject}  spp={args.spp}  size={args.size}px")
    print(f"Physically grounded baseline — static calibrated parameters, no noise modulation")
    print(f"{'='*65}")
    ...
    for theta in offsets:
        ...
        simple_path = out_dir / f"simple_v19_{label}.png"
        ...
        gt_path = out_dir / f"gt_v19_spp{args.spp}_{label}.png"
    ...
    grid_path = pair_out / f"{args.subject}_v19_cinematic_hd_spp{args.spp}.png"
