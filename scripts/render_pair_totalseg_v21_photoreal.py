"""
render_pair_totalseg_v21_photoreal.py — Photorealism pass over v20

v20 renders cleanly but reads as painted latex. This version fixes the four things
that actually cause that read. Everything is a deterministic function of object-space
...
Run:
    blender --background --python scripts/render_pair_totalseg_v21_photoreal.py -- \\
...
def get_args():
    ...
    ap.add_argument("--saturation", type=float, default=1.25,
                    help="pre-compensation for AgX inset desaturation (v20 used 0.95)")
    ...
    ap.add_argument("--look",       default="AgX - Punchy",
                    help="v20 used 'AgX - Medium Contrast'")
...
def build_micro_normals(nodes, links, vec_out, t, base_normal_out, detail_gain):
    ...
    c_mid  = _noise(nodes, links, vec_out, 3.0, _phase(name, 8), detail=4.0)
    c_fine = _noise(nodes, links, vec_out, 1.0, _phase(name, 9), detail=5.0)
...
def make_material(t, feat):
    ...
    if vessel_height is not None:
        vb = _bump(nodes, links, vessel_height, normal_out, 0.25, 0.00022)
