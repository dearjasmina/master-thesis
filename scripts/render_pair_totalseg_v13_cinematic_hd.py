...
TISSUES = [
    ...
    ("autochthon_left",   "#C05A28", [0.12,0.04,0.03], 0.35, 1.40, 0.03, 1.5, (1.0,0.1,0.02),  0.15, 0.10, "fibrous",  0.25, "plane_xz"),
    ("autochthon_right",  "#C05A28", [0.12,0.04,0.03], 0.35, 1.40, 0.03, 1.5, (1.0,0.1,0.02),  0.15, 0.10, "fibrous",  0.25, "plane_xz"),
    # Lungs: dark blood-perfused, tight SSS to prevent paper-lantern glowing
    ("lung_lower_lobe_left",  "#88AABC", [0.16,0.08,0.09], 0.40, 1.35, 0.04, 2.0, (1.2,0.2,0.05), 0.15, 0.15, "smooth", 0.15, "sphere"),
    ("lung_lower_lobe_right", "#88AABC", [0.16,0.08,0.09], 0.40, 1.35, 0.04, 2.0, (1.2,0.2,0.05), 0.15, 0.15, "smooth", 0.15, "sphere"),
    ("lung_upper_lobe_left",  "#88AABC", [0.16,0.08,0.09], 0.40, 1.35, 0.04, 2.0, (1.2,0.2,0.05), 0.15, 0.15, "smooth", 0.15, "sphere"),
    ("lung_upper_lobe_right", "#88AABC", [0.16,0.08,0.09], 0.40, 1.35, 0.04, 2.0, (1.2,0.2,0.05), 0.15, 0.15, "smooth", 0.15, "sphere"),
    ...
    ("heart",      "#B03030", [0.28,0.06,0.05], 0.28, 1.40, 0.04, 3.0, (1.2,0.15,0.05), 0.25, 0.06, "lobular", 0.45, "sphere"),
    ("esophagus",  "#C87878", [0.35,0.15,0.14], 0.35, 1.40, 0.04, 2.0, (1.0,0.2,0.08),  0.15, 0.08, "vessel",  0.25, "plane_xz"),
    # Liver: deep crimson anchor — darkest organ, maximum value separation from bowel
    ("liver",      "#9E3028", [0.07,0.015,0.012], 0.22, 1.38, 0.07, 2.0, (1.5,0.1,0.02), 0.40, 0.04, "lobular", 0.50, "sphere"),
    # Stomach: olive/ochre — contrasts against deep liver
    ("stomach",    "#E8C866", [0.38,0.32,0.22], 0.30, 1.40, 0.06, 2.5, (1.0,0.4,0.1),   0.18, 0.08, "wrinkled",0.50, "sphere"),
    # Gallbladder: dark forest green
    ("gallbladder","#60A840", [0.08,0.18,0.05], 0.15, 1.40, 0.06, 2.0, (0.2,1.0,0.2),   0.35, 0.04, "lobular", 0.30, "sphere"),
    # Spleen: deep purplish-red
    ("spleen",     "#7B3A8C", [0.18,0.03,0.08], 0.20, 1.40, 0.06, 2.5, (1.2,0.1,0.15),  0.35, 0.04, "lobular", 0.45, "sphere"),
    # Kidneys
    ("kidney_right","#2A5EA8",[0.15,0.06,0.10], 0.22, 1.42, 0.04, 2.5, (1.2,0.2,0.1),   0.30, 0.05, "lobular", 0.50, "sphere"),
    ("kidney_left", "#2A6EB8",[0.15,0.06,0.10], 0.22, 1.42, 0.04, 2.5, (1.2,0.2,0.1),   0.30, 0.05, "lobular", 0.50, "sphere"),
    # Bowel group: tan/ochre — brightest tonal band, pops against deep liver
    ("pancreas",   "#E8B870", [0.42,0.30,0.16], 0.38, 1.40, 0.04, 2.0, (1.0,0.5,0.2),   0.18, 0.08, "lobular", 0.45, "sphere"),
    ("duodenum",   "#D4A080", [0.38,0.24,0.16], 0.35, 1.40, 0.03, 2.5, (0.9,0.3,0.1),   0.18, 0.08, "wrinkled",0.45, "plane_xy"),
    ("small_bowel","#D4A080", [0.38,0.24,0.16], 0.35, 1.40, 0.03, 2.5, (0.9,0.3,0.1),   0.18, 0.08, "wrinkled",0.45, "sphere"),
    ("colon",      "#C88060", [0.34,0.20,0.14], 0.35, 1.40, 0.03, 2.5, (0.8,0.3,0.1),   0.18, 0.08, "wrinkled",0.45, "sphere"),
    ("urinary_bladder","#7888D4",[0.16,0.18,0.35],0.25,1.40,0.04, 2.0, (0.4,0.4,1.0),   0.25, 0.05, "smooth",  0.20, "sphere"),
    # Vessels: pure saturated red/blue — brightest specular under key
    ("aorta",                    "#DD1818",[0.45,0.03,0.02],0.10,1.38,0.08,1.5,(2.0,0.1,0.02),0.40,0.03,"vessel",0.20,"plane_xz"),
    ("inferior_vena_cava",       "#1840A0",[0.02,0.06,0.30],0.12,1.38,0.06,1.5,(0.1,0.2,2.0), 0.35,0.03,"vessel",0.15,"plane_xz"),
    ("portal_vein_and_splenic_vein","#2050B8",[0.03,0.08,0.28],0.12,1.38,0.06,1.5,(0.1,0.2,2.0),0.30,0.03,"vessel",0.15,"plane_xy"),
    ("superior_vena_cava",       "#1840A0",[0.02,0.06,0.30],0.12,1.38,0.06,1.5,(0.1,0.2,2.0), 0.35,0.03,"vessel",0.15,"plane_xz"),
]
...
def setup_render(spp, size, device):
    ...
    scene.cycles.diffuse_bounces         = 3   # 8 caused warm bounce flooding that pushed scene pink
    scene.cycles.glossy_bounces          = 4
    scene.cycles.transmission_bounces    = 12
    scene.cycles.volume_bounces          = 6
    scene.cycles.transparent_max_bounces = 24
    scene.cycles.blur_glossy             = 0.1   # stable specular — 0.05 caused noisy glitter without denoiser

    # ── V13: Pixel filter — BOX at 0.5 is the sharpest possible Cycles output ──
    scene.cycles.pixel_filter_type = 'BOX'
    scene.cycles.filter_width      = 0.5   # mathematical sub-pixel precision
...
def setup_compositor(scene):
    """V13: High-pass edge sharpening + filmic S-curve tone grade."""
    try:
        ...
        # Clean pass-through — LINEAR_LIGHT sharpening amplified halos on red glossy surfaces
...
def make_material(seg_name, base_rgb, roughness, ior,
    ...
    ao_node.inputs['Distance'].default_value = 0.0035  # 3.5mm — tighter crease separation, less ambient spill
    mix_ao = nodes.new('ShaderNodeMix')
    mix_ao.data_type  = 'RGBA'
    mix_ao.blend_type = 'MULTIPLY'
    mix_ao.inputs['Factor'].default_value = 0.32       # 32% — stronger contact shadow stamp
    ...
    if any(w in seg_name for w in _wrinkled):
        noise_r.inputs['Scale'].default_value  = 45.0  # tighter wrinkle frequency for bowel
        noise_r.inputs['Detail'].default_value = 12.0
    else:
        noise_r.inputs['Scale'].default_value  = 20.0  # broad organic variation for solid organs
        noise_r.inputs['Detail'].default_value = 8.0
    noise_r.inputs['Roughness'].default_value  = 0.65
    noise_r.inputs['Distortion'].default_value = 0.20
    ...
    val_map.inputs['To Min'].default_value   = roughness * 0.35
    ...
    # ── Random Walk SSS ───────────────────────────────────────────────────────
    # Skip on hollow thin-walled structures — SSS bleeds through thin mesh edges
    ...
    if sss_weight > 0 and not any(h in seg_name for h in _hollow):
        ...
        principled.inputs['Subsurface Scale'].default_value  = sss_scale_mm / 1000.0 * 0.003   # 0.0015 was too low, stripped depth color → chalky pink
        principled.inputs['Subsurface Radius'].default_value = sss_radius

    # ── Coat: peritoneal fluid — per-tissue weight, concentrated wet highlights ─
    if coat_weight > 0:
        principled.inputs['Coat Weight'].default_value    = coat_weight
        principled.inputs['Coat Roughness'].default_value = coat_roughness  # per-tissue: vessels 0.03, bowel 0.08, organs 0.04-0.06
    ...
    bevel_node.inputs['Radius'].default_value = 0.00012  # uniform edge catch — enough for specular without over-sharpening
    ...
    if bump_type != "none" and bump_scale > 0:
        ...
        if bump_path.exists():
            ...
            bump_n.inputs['Strength'].default_value = bump_scale * 1.2  # enhanced local contrast
            bump_n.inputs['Distance'].default_value = 0.00025           # deeper groove = reads at distance
...
def setup_lights(cx, cy, cz, scene_scale):
    ...
    key.data.energy = 110
    key.data.color  = (0.82, 0.88, 1.00)  # cooler blue-white — less warm spill
    key.data.size   = sc * 0.095  # sc*0.045 without denoiser = noisy glitter, not crispness
    ...
    fill.data.energy = 5.0   # 0.8W caused complete cavity blackout — needs slight lift for readability
    ...
    rim1.data.energy = 45   # 150W was too strong, overpowering cool key contrast
    ...
    rim2.data.energy = 120  # stronger cool rim to peel dark organs from void
...
def setup_camera(size, fov_deg=20):  # 20° = cinematic lens compression
    ...
    # DoF: f/8 — deep focus macro, foreground organs tack-sharp
...
def point_camera(cam_obj, position, target):
    ...
    cam_obj.data.dof.focus_distance = dist * 0.93  # pull focus 7% forward — catches foreground organs
...
def main():
    ...
    for theta in offsets:
        ...
        simple_path = out_dir / f"simple_v13_{label}.png"
        ...
        gt_path = out_dir / f"gt_v13_spp{args.spp}_{label}.png"
    ...
    grid_path = pair_out / f"{args.subject}_v13_cinematic_hd_spp{args.spp}.png"
