"""
render_pair_totalseg_v17_cinematic_hd.py — Specular Coherence Break + Macro Deformation

v17 upgrades over v16:
  - Directional coat normal perturbation: anisotropic stretched noise (scale=(6,1.5,6), noise=15)
    → ShaderNodeBump (strength=0.15) → Principled Coat Normal — breaks specular coherence
  - Chromatic SSS scale variation: object-space noise (scale=5) modulates SSS depth ±25% per point
  - Macro deformation bump: very low-freq (scale=1.2) anisotropic object-space noise stacked on
    bevel node, before texture bump chain — organic surface deformation feel
  - Rim2 demoted: 35W→21W (-40%), size sc*0.02→sc*0.08 — reduces stylized specular catchlight

v16 upgrades (retained):
  - Low-freq color variation on vascular organs (liver/kidney/spleen/heart): object-space noise
    scale=4, multiplied into base color 0.88-1.12 — bruised depth, dark patches
  - Coat noise modulation 0.85×-1.15× + AO concavity boost (+25% coat in occluded cavities)
  - Multi-scale roughness: macro (scale=2.5) × mid × micro (scale=85) — layered biological chaos
  - AO curvature proxy: convex surfaces drier (1.18×), concave wetter (0.82×) — stops uniform sheen
  - Subtle compositor fog-glow (mix=-0.97, threshold=0.88): breaks optical perfection in specular peaks

Run:
    blender --background --python scripts/render_pair_totalseg_v17_cinematic_hd.py -- \\
...
def make_material(seg_name, base_rgb, roughness, ior,
    ...
    # Macro layer — broad regional dryness patches (scale=2.5, very low freq)
    noise_macro = nodes.new('ShaderNodeTexNoise')
    noise_macro.inputs['Scale'].default_value     = 2.5
    ...
    if sss_weight > 0 and not any(h in seg_name for h in _hollow):
        ...
        # Chromatic SSS scale variation: noise-modulated depth ±25% per surface point
        obj_map_sss = nodes.new('ShaderNodeMapping')
        obj_map_sss.inputs['Scale'].default_value = (1.0, 1.0, 1.0)
        links.new(uv_node.outputs['Object'], obj_map_sss.inputs['Vector'])

        noise_sss = nodes.new('ShaderNodeTexNoise')
        noise_sss.inputs['Scale'].default_value     = 5.0
        noise_sss.inputs['Detail'].default_value    = 2.0
        noise_sss.inputs['Roughness'].default_value = 0.5
        links.new(obj_map_sss.outputs['Vector'], noise_sss.inputs['Vector'])

        base_sss = sss_scale_mm * 0.001
        map_sss = nodes.new('ShaderNodeMapRange')
        map_sss.inputs['To Min'].default_value = base_sss * 0.75
        map_sss.inputs['To Max'].default_value = base_sss * 1.25
        links.new(noise_sss.outputs['Fac'], map_sss.inputs['Value'])
        links.new(map_sss.outputs['Result'], principled.inputs['Subsurface Scale'])
    ...
    if coat_weight > 0:
        ...
        # AO concavity boost: occluded cavities stay wetter → higher coat (up to +25%)
        ...
        bump_coat_n.inputs['Strength'].default_value = 0.15
        bump_coat_n.inputs['Distance'].default_value = 0.0003
    ...
    bump_def.inputs['Strength'].default_value = 0.45
    bump_def.inputs['Distance'].default_value = 0.0008
...
def main():
    ...
    print(f"\n{'='*65}")
    print(f"[v17-cinematic-hd]  subject={args.subject}  spp={args.spp}  size={args.size}px")
    print(f"Specular coherence break + macro deformation + SSS variation")
    print(f"{'='*65}")
    ...
    for theta in offsets:
        ...
        simple_path = out_dir / f"simple_v17_{label}.png"
        ...
        gt_path = out_dir / f"gt_v17_spp{args.spp}_{label}.png"
    ...
    grid_path = pair_out / f"{args.subject}_v17_cinematic_hd_spp{args.spp}.png"
