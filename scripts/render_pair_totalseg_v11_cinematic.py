"""
render_pair_totalseg_v11_cinematic.py — Ultra-Cinematic Blender Cycles

Cinematic upgrades over v10:
  - VolumeAbsorption inside SSS organs (internal depth + color filtering per organ)
  - Max light path bounces: diffuse=8, glossy=8, transmission=12, SSS=12, volume=6
  - Shallow depth of field (f/2.8, auto-focused on scene centre)
  - Negative fill planes (black absorption cards — sculpts shadow contrast)
  - World volumetric scatter (density=0.003 warm haze — depth separation + light shafts)
  - Roughness noise texture (breaks up uniform surface → imperfect wet tissue)
  - Compositor: subtle bloom on specular highlights
  - 256 SPP at 1024px (production quality; no runtime budget)

Run:
    blender --background --python scripts/render_pair_totalseg_v11_cinematic.py -- \\
        --subject s0050 --spp 256 --size 1024 --angles 3

Meshes must exist first (generate once with v7 script):
...
def make_material(seg_name, base_rgb, roughness, ior,
    ...
    # ── Roughness with noise variation (breaks up uniform surface) ────────────
    noise_r = nodes.new('ShaderNodeTexNoise')
    noise_r.inputs['Scale'].default_value      = 45.0   # high-frequency = micro wet variation
    ...
    val_map.inputs['To Max'].default_value   = roughness * 0.7
    ...
    if sss_weight > 0:
        ...
        principled.inputs['Subsurface Scale'].default_value  = sss_scale_mm / 1000.0 * 0.012
        principled.inputs['Subsurface Radius'].default_value = sss_radius

    # ── Coat: peritoneal fluid / Glisson capsule ──────────────────────────────
    if coat_weight > 0:
        principled.inputs['Coat Weight'].default_value    = 1.0     # full peritoneal fluid coating — all organs are wet
        principled.inputs['Coat Roughness'].default_value = 0.015   # razor-sharp glassy reflection, not greasy smear
        principled.inputs['Coat IOR'].default_value       = 1.41    # biological fluid refraction
    ...
    if bump_type != "none" and bump_scale > 0:
        ...
        if bump_path.exists():
            ...
            bump_n.inputs['Distance'].default_value = 0.00012  # 0.12mm — tight micro-detail
...
def setup_lights(cx, cy, cz, scene_scale):
    ...
    # Key: grazing incidence from upper-side — reveals surface topology, sculpts depth
    bpy.ops.object.light_add(type='AREA',
        location=(cx + sc*1.4, cy + sc*0.3, cz + sc*0.8))
    key = bpy.context.object
    key.data.energy = 120
    key.data.color  = (0.92, 0.95, 1.00)  # cool-neutral — organ albedo provides warmth
    key.data.size   = sc * 0.05  # tiny = pin-sharp specular highlights on wet surfaces
    key.data.shape  = 'SQUARE'
    _track_to(key, (cx, cy, cz))

    # Fill: minimal — just lifts absolute black, preserves shadow drama
    bpy.ops.object.light_add(type='AREA',
        location=(cx - sc*1.0, cy - sc*1.0, cz + sc*0.5))
    fill = bpy.context.object
    fill.data.energy = 2.5   # near-zero — cavities fall into shadow for depth
    fill.data.color  = (0.60, 0.75, 1.00)  # cool blue — colour contrast against warm key
    fill.data.size   = sc * 0.50
    _track_to(fill, (cx, cy, cz))

    # Rim: razor-thin warm edge — silhouette separation, not diffuse bleed
    bpy.ops.object.light_add(type='AREA',
        location=(cx - sc*0.4, cy + sc*1.3, cz + sc*0.2))
    rim = bpy.context.object
    rim.data.energy = 130
    rim.data.color  = (1.00, 0.55, 0.35)  # rich amber/crimson
    rim.data.size   = sc * 0.04  # razor tight = sharp edge highlight
...
def setup_camera(size, fov_deg=28):
    ...
    # DoF: f/2 — shallow cinematic bokeh, foreground/background organs blur
    cam_obj.data.dof.use_dof        = True
    cam_obj.data.dof.aperture_fstop = 2.0
    return cam_obj
...
def main():
    ...
    for theta in offsets:
        ...
        simple_path = out_dir / f"simple_v11_{label}.png"
        ...
        gt_path = out_dir / f"gt_v11_spp{args.spp}_{label}.png"
    ...
    grid_path = pair_out / f"{args.subject}_v11_cinematic_spp{args.spp}.png"
